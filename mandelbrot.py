...
def doplot(toplot,cX,cY,lX,lY,n):
    # plots a colour map    
    fig = plt.imshow(toplot,extent = [cX-lX/2 ,cX+lX/2,cY-lY/2,cY+lY/2], cmap = CM.jet)
    plt.axis('off')
    fig.axes.get_xaxis().set_visible(False)
    fig.axes.get_yaxis().set_visible(False)
    #plt.savefig('foo'+str(n)+'.png', bbox_inches='tight', pad_inches = 0,)

def savenumbers(ans,i,nX,nY):
    tempfile = open('MandelNumbers' + str(i)+'.txt','w')
    for i in range(nX):
        for j in range(nY):
...
def domultiplemandel(nX,nY,cX,cY,lX,lY,MAX_ITERATIONS):
    for i in range(10):
        [lX,lY] = [0.5*lX,0.5*lY] # Total length of the X, Y axis
...
def main():
    ...
    [cX,cY] = [0.25,0] # Middle of where we are looking
    [lX,lY] = [4,4] # Total length of the X, Y axis
    domultiplemandel(nX,nY,cX,cY,lX,lY,MAX_ITERATIONS)
    #abc = loadnumbers('MandelNumbers',0,nX,nY)
    #doplot(abc,cX,cY,lX,lY,0)
    #print(abc)

#main()
loadandplot('MandelNumbers',10,500,500)